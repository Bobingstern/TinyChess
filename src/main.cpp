...
const uint16_t BOOK_MOVES[] = {
    ...
    movePack(D2, D4, true, false, 0, false, 0),
    movePack(D7, D5, true, false, 0, false, 0),
    movePack(C2, C4, true, false, 0, false, 0),
    movePack(D5, C4, false, false, 0, true, 0),
...
//cutechess-cli.exe -engine conf=TinyChess1 -engine conf=TinyChess0 -each proto=uci tc=inf -sprt elo0=0 elo1=5 alpha=0.05 beta=0.05 -games 200 -openings file=Silver_Suite.pgn format=pgn plies=5
  ...
  std::string Line; //to read the command given by the GUI
	...
	while( std::getline(std::cin, Line ) ) {
		if ( Line == "uci" ) {
			...
			std::cout << "id author Anik Patel and Ckyiu" << std::endl;
