...
float Engine::alphaBeta(float alpha, float beta, uint64_t attackers, int depthleft, int originalDepth, uint16_t &bestMove, bool &left){
  if (depthleft == 0) {
  ...
  for (int i = 0; i < total; i++) {
    ...
    noLegal = false;
    // Do stuff here
    score = -alphaBeta(-beta, -alpha, a, depthleft-1, originalDepth, bestMove, left);
    //std::cout << "\n";
    this->board->unmakeMove(moves[i]);
...
uint16_t Engine::runSearchID(int m){
  ...
  for (int i=1;i<10;i++){
    ...
    alphaBeta(-10000, 10000, board->getAttackers(), i, i, bestMove, done);
    if (i == 1){
...
uint16_t Engine::runSearch(int depth, int m) {
  ...
  bool done = false;
  maxTime = m;
  alphaBeta(-10000, 10000, board->getAttackers(), depth, depth, bestMove, done);
  return bestMove;
}